  ...
  <div v-if="movie" class="container mx-auto px-4 py-8">
    ...
    <div v-if="movie" class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
      ...
      <div>
        ...
        <p v-if=" movie.status" class="text-white">
          <span class="text-gray-400">Статус: </span> {{ movie.status? movie.status: неизвестнно }}
        </p>
        <p v-if="movie.budget&&movie.budget.value"  class="text-white">
          <span class="text-gray-400">Бюджет: </span>
          {{ movie.budget ? `${movie.budget.value}${movie.budget.currency}` : "Неизвестно" }}
        </p>
        <p class="text-white">
